...
Important note: Envox d.o.o. does not claim any ownership rights to the source code generated by the `Build` command. The user owns the `.eez-project` file, all templates and all files generated by EEZ studio from the `.eez-project` and template files and can choose any license that suits them best.
