...
static inline const char *eez_string_from_double(double value) {
    ...
    eez_string_free(result);
    char buf[32];
    lv_snprintf(buf, sizeof(buf), "%.17g", value);
    eez_string_assign(result, buf);
    return eez_string_cstr(result);
}
...
//
typedef struct {
    EezString master_pin;  // Global: MASTER_PIN
} FlowGlobalVariables;
...
//
// Flow state for screen 'Enter_PIN'
// Contains local variables for this screen
//
typedef struct {
    EezString pin;  // Local: pin
    EezString error;  // Local: error
} FlowState_enter_pin;
...
//
// Flow state for screen 'counter'
// Contains local variables for this screen
//
typedef struct {
    int32_t cnt;  // Local: cnt
} FlowState_counter;
