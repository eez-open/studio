...
#ifndef EEZ_MAX_TEMP_STRINGS
#define EEZ_MAX_TEMP_STRINGS 4
#endif

// Temporary string buffer for intermediate results
static EezString _eez_str_temp[EEZ_MAX_TEMP_STRINGS];
...
static inline int32_t eez_string_find(const char *str, const char *substr) {
    if (str == NULL || substr == NULL) return -1;
    const char *found = strstr(str, substr);
    if (found == NULL) return -1;
    return (int32_t)(found - str);
}
...
static inline const char *eez_string_from_float(float value) {
    ...
    eez_string_free(result);
    char buf[32];
    lv_snprintf(buf, sizeof(buf), "%g", (double)value);
    eez_string_assign(result, buf);
    return eez_string_cstr(result);
}

// Convert double to string, returns C string from temp buffer
static inline const char *eez_string_from_double(double value) {
    EezString *result = _eez_str_get_temp();
    eez_string_free(result);
    char buf[32];
    lv_snprintf(buf, sizeof(buf), "%g", value);
    eez_string_assign(result, buf);
    return eez_string_cstr(result);
}
...
//
// Flow state for screen 'Calculator'
// Contains local variables for this screen
//
typedef struct {
    EezString display;  // Local: display
    double operand;  // Local: operand
    EezString operator;  // Local: operator
    bool new_number;  // Local: newNumber
} FlowState_calculator;
