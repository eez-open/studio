...
const TimelineAnimationCurve = observer(
    ({ flowContext }: { flowContext: EditorFlowContext }) => {
        ...
        if (timeline.selectedKeyframes.length != 1) {
            return null;
        }

        const keyframe = timeline.selectedKeyframes[0];
        ...
        const CURVE_STROKE_WIDTH = 3;
        const CURVE_COLOR = "green";

        const LINE_TO_CONTROL_POINT_STROKE_WIDTH = 1;
        const LINE_TO_CONTROL_POINT_COLOR = "green";

        const CONTROL_POINT_HANDLE_COLOR = "green";

        if (controlPointsArray.length == 4) {
            const p1 = {
                x: getKeyframesFromPropertyValue(
                    timeline.selectedKeyframes,
                    "left"
                ),
                y: getKeyframesFromPropertyValue(
                    timeline.selectedKeyframes,
                    "top"
                )
            };
            ...
            const p4 = {
                x: getKeyframesPropertyValue(
                    timeline.selectedKeyframes,
                    "left"
                ),
                y: getKeyframesPropertyValue(timeline.selectedKeyframes, "top")
            };
                ...
                return (
                    <Svg flowContext={flowContext}>
                        ...
                        <path
                            ...
                            strokeWidth={LINE_TO_CONTROL_POINT_STROKE_WIDTH}
                        ...
                        <circle
                            ...
                            cy={p2.y}
                            r={8}
                            fill={CONTROL_POINT_HANDLE_COLOR}
                        />
                        <circle
                            cx={p3.x}
                            cy={p3.y}
