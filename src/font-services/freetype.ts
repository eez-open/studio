...
class FreeTypeFile {
    ...
    load(): boolean {
        ...
        this.create_font_extract_state = wasmModule.cwrap("create_font_extract_state", "number", [
            "string",
            "number",
            "number",
            "number"
        ...
        this.free_font_extract_state = wasmModule.cwrap("free_font_extract_state", "number", [
            "number"
        ]);
        ...
        this.glyphPixelsDataPtr = wasmModule._malloc(this.glyphPixelsArrayLength);
    ...
    addGlyph(encoding: number, createBlankGlyphs: boolean) {
        ...
        if (!createBlankGlyphs) {
            ...
            if (result) {
                ...
                const glyphInfoAdvanceX = glyphInfoDataView.getFloat64(16, true);
                ...
                if (hasPixels) {
                    ...
                    if (this.bpp === 8) {
                        pixelArray = new Array<number>(glyphWidth * glyphHeight);
                        let i = 0;
                        for (
                            ...
                            for (
                                ...
                                pixelArray[i++] = this.glyphPixelsArray[
                        ...
                        for (
                            ...
                            for (
                                let x = this.glyphPosition.x + glyphInfoX1, iByte = 0;
                                iByte < widthInBytes && x < this.glyphPosition.x + glyphInfoX2;
                                iByte++
                            ) {
                                let byteData = 0;
                                ...
                                for (
                                    let iBit = 0;
                                    iBit < 8 && x < this.glyphPosition.x + glyphInfoX2;
                                    iBit++, x++
                                ) {
                                    if (
                                        this.glyphPixelsArray[(y * this.canvasWidth + x) * 4 + 3] >
                                        this.threshold
                                    ) {
                                        byteData |= 0x80 >> iBit;
                                    }
...
export async function extractFreeTypeFont(
    ...
    try {
        ...
        if (createGlyphs) {
            ...
            if (fromEncoding && toEncoding) {
                encodings = _range(fromEncoding, toEncoding + 1);
            } else {
                encodings = await getAllGlyphEncodings(absoluteFilePath);
            }

            for (let i = 0; i < encodings.length; i++) {
                // give control to JavaScript engine, we don't want to block main thread
                await new Promise(resolve => setTimeout(resolve));
                freeTypeFile.addGlyph(encodings[i], createBlankGlyphs);
            }
        }
...
export async function rebuildFont(font: FontProperties, projectFilePath: string) {
    // wait until fontExtract module is ready
    while (!wasmModuleReady) {
        await new Promise(resolve => setTimeout(resolve));
    }
    ...
    try {
        ...
        for (let i = 0; i < font.glyphs.length; ++i) {
            ...
            if (!glyph.glyphBitmap || glyph.glyphBitmap.pixelArray.length == 0) {
                glyphs.push(glyph);
                continue;
            }
            ...
            let freeTypeFile = freeTypeFileMap.get(glyph.source.filePath + ":" + glyph.source.size);
            if (!freeTypeFile) {
                ...
                freeTypeFileMap.set(glyph.source.filePath + ":" + glyph.source.size, freeTypeFile);
            }

            const newGlyph = freeTypeFile.addGlyph(glyph.source.encoding || glyph.encoding, false);
